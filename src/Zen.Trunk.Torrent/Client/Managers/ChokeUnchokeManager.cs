	...
	internal class ChokeUnchokeManager
	{
		...
		public void TimePassed()
		{
			...
			if (firstCall)
			{
				...
				isDownloading = !owningTorrent.IsComplete; //If progress is less than 100% we must be downloading
				firstCall = false;
				ExecuteReview();
			}
			else if (isDownloading && owningTorrent.IsComplete)
			{
			...
			else if (minimumTimeBetweenReviews > 0 && (SecondsBetween(timeOfLastReview, DateTime.UtcNow) >= minimumTimeBetweenReviews) &&
		...
		private void ExecuteReview()
		{
			...
			//    logEntry.Append(B2YN(owningTorrent.State == TorrentState.Seeding) + timeOfLastReview.ToString() + "," + DateTime.UtcNow.ToString() + ";");
			...
			//                            SecondsBetween(lastUnchoked.Value, DateTime.UtcNow).ToString());
			//                    logEntry.Append(";");
			//                }
			//            }
			//    }
			...
			foreach (PeerId connectedPeer in owningTorrent.Peers.ConnectedPeers)
			{
				if (connectedPeer.Connection != null)
				{
					if (!connectedPeer.Peer.IsSeeder)
					{
						...
						double timeSinceLastReview = SecondsBetween(timeOfLastReview, DateTime.UtcNow);
						double timeUnchoked = 0;
						if (!connectedPeer.AmChoking)
						{
							timeUnchoked = SecondsBetween(connectedPeer.LastUnchoked.Value, DateTime.UtcNow);
							unchokedPeers++;
						}
			...
			timeOfLastReview = DateTime.UtcNow;
			reviewsExecuted++;
		}
		...
		private void ExecuteTyrantReview()
		{
			...
			this.timeOfLastReview = DateTime.UtcNow;
		...
		private void Unchoke(PeerId PeerToUnchoke)
		{
			...
			PeerToUnchoke.LastUnchoked = DateTime.UtcNow;
		...
		//        }
		//        logStreamWriter.WriteLine(DateTime.UtcNow.ToString() + ":" + LogEntry);
		//    }
		//}
