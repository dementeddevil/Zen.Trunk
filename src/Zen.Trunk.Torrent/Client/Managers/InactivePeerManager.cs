namespace Zen.Trunk.Torrent.Client
{
	...
	internal class InactivePeerManager
	{
		...
		public void TimePassed()
		{
			...
			if (owningTorrent.Settings.MaximumConnections > owningTorrent.OpenConnections)
			{
				return;
			}
			...
			for (int i = 0; i < owningTorrent.Peers.ConnectedPeers.Count; i++)
			{
				...
				if (nextPeer.BytesReceived == 0 && nextPeer.WhenConnected.AddSeconds(timeToWaitBeforeIdle) < DateTime.UtcNow)
				{
