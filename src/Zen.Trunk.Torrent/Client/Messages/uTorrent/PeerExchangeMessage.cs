namespace Zen.Trunk.Torrent.Client.Messages.Libtorrent
{
	...
	public class PeerExchangeMessage : ExtensionMessage
	{
		...
		public byte[] Added
		{
			set
			{
				peerDict[AddedKey] = (BEncodedString)(value ?? ZeroArray);
			}
			get
			{
				return ((BEncodedString)peerDict[AddedKey]).TextBytes;
			}
		}
		...
		public byte[] AddedDotF
		{
			set
			{
				peerDict[AddedDotFKey] = (BEncodedString)(value ?? ZeroArray);
			}
			get
			{
				return ((BEncodedString)peerDict[AddedDotFKey]).TextBytes;
			}
		}
		...
		public byte[] Dropped
		{
			set
			{
				peerDict[DroppedKey] = (BEncodedString)(value ?? ZeroArray);
			}
			get
			{
				return ((BEncodedString)peerDict[DroppedKey]).TextBytes;
			}
		}
		...
		public override int ByteLength
		{
			get
			{
				return 4 + 1 + 1 + peerDict.LengthInBytes();
			}
		}
